...
class SemiconductorApp {
  ...
  processCsvString(csvText, filename = '', fileSize = null) {
    try {
      ...
      if (parsed.type === 'repeatability') {
        ...
        this.state.itemLabelMode = 'AUTO';
        this.recomputeItemAnalysis();
        this.updateItemUi();
        targetTab = 'item';
  ...
  /**
   * A single Item Compare CSV can mix OLD_vs_NEW and PROPOSED_vs_NEW rows (Column A
   * is per-row). "Auto-detect" shows every row combined, exactly as loaded. Explicitly
   * choosing OLD vs NEW or NEW vs PROPOSE actually filters down to just that row
   * population — Total Records and every other stat then reflect only that subset,
   * not the whole file.
   */
  getItemFilteredRows() {
    const rows = this.state.itemFile?.rows || [];
    const mode = this.state.itemLabelMode || 'AUTO';
    if (mode === 'OLD_NEW') {
      return rows.filter(r => !String(r.Comparison || '').toUpperCase().includes('PROPOSED'));
    }
    if (mode === 'NEW_PROPOSE') {
      return rows.filter(r => String(r.Comparison || '').toUpperCase().includes('PROPOSED'));
    }
  ...
  recomputeItemAnalysis() {
    if (!this.state.itemFile) return;
    const filteredFile = { ...this.state.itemFile, rows: this.getItemFilteredRows() };
    this.state.itemAnalysis = SemiconductorAnalytics.analyzeItemCompare(filteredFile, this.state.testIgnoreRules);
  }
  ...
  updateItemUi() {
    const item = this.state.itemAnalysis;
    const raw = this.getItemFilteredRows();
    if (!item) return;
